...
//   const handleUpdate = async (id, montantPaye) => {
//     if (!editingInvoice) return;

//     try {

//       const montantPaye = montantPaye === 0 ?null:new Date()

//       await axios.put(`http://127.0.0.1:3320/api/invoice/updateOne/${id}`, { montantPaye });
//       fetchInvoices();
...
//                   <input
//                     type="number"
//                     value={invoice.montantPaye || 0}
//                     onChange={(e) =>
//                       setInvoices((prev) =>
//                         prev.map((inv) =>
//                           inv._id === invoice._id ? { ...inv, montantPaye: parseFloat(e.target.value) || 0 } : inv
//                         )
//                       )
//                     }
//                   />
...
//               <td>{formatDateTime(invoice.datePaiementEntreprise) || "Non défini"}</td>
//               <td> {invoice.datePaiementClient === null ? "No Payment Yet": formatDateTime(invoice.datePaiementClient)}</td>
//               <td>{invoice.statut}</td>
//               <td>
//                 {invoice.statut === "paid" ? null : editingInvoiceId === invoice._id ? (
//                   <button onClick={() => handleUpdate(invoice._id, invoice.montantPaye)}>Enregistrer</button>
//                 ) : (

//                   <button onClick={() => handleEditClick(invoice._id)}>Modifier</button>
//                 )}
